...
export default function EmpleadoUser() {
  ...
  const handleStartService = async (ticketId: string) => {
    ...
    try {
      ...
      // Try to auto-assign next ticket after starting service
      setTimeout(async () => {
        try {
  ...
  const handleCompleteTicket = async (ticketId: string, callNext: boolean = false) => {
    ...
    try {
      ...
      if (callNext) {
        // Use the auto-assign function for next ticket
        setTimeout(async () => {
          try {
            await autoAssignNextTicket(currentEmployee.id);
          } catch (error) {
            ...
            <div className="bg-gradient-to-r from-blue-50 to-green-50 rounded-xl p-6">
              ...
              <div className="flex space-x-3">
                ...
                <button
                  ...
                  className="flex-1 bg-purple-500 hover:bg-purple-600 text-white py-3 px-4 rounded-xl font-semibold transition-colors flex items-center justify-center space-x-2"
                >
                  <ArrowRight size={20} />
                  <span>Derivar</span>
                </button>
            ...
            <div className="mb-6">
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <button
                    onClick={() => handleDeriveTicket('queue')}
                    disabled={isLoading}
                    className="bg-blue-500 hover:bg-blue-600 disabled:bg-blue-300 text-white py-3 px-4 rounded-lg font-semibold transition-colors flex items-center justify-center space-x-2"
                  >
                    <Users size={20} />
                    <span>A Cola General</span>
                  </button>
                  
                  <button
                    onClick={() => {
                      // For simplicity, derive to first available employee
                      const availableEmployee = state.employees.find(emp => 
                        emp.isActive && emp.id !== currentEmployee?.id
                      );
                      if (availableEmployee) {
                        handleDeriveTicket('employee', availableEmployee.id);
                      } else {
                        alert('No hay empleados disponibles');
                      }
                    }}
                    disabled={isLoading}
                    className="bg-purple-500 hover:bg-purple-600 disabled:bg-purple-300 text-white py-3 px-4 rounded-lg font-semibold transition-colors flex items-center justify-center space-x-2"
                  >
                    <User size={20} />
