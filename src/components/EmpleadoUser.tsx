...
export default function EmpleadoUser() {
  ...
  const handleStartService = async (ticketId: string) => {
    ...
    // Get all waiting tickets (both general and personal queue)
    const allWaitingTickets = state.tickets
      .filter(t => t.status === 'waiting')
      .sort((a, b) => {
        // Personal queue tickets have priority
        const aIsPersonal = t.queueType === 'personal' && t.assignedToEmployee === currentEmployee.id;
        const bIsPersonal = t.queueType === 'personal' && t.assignedToEmployee === currentEmployee.id;
        
        if (aIsPersonal && !bIsPersonal) return -1;
        if (!aIsPersonal && bIsPersonal) return 1;
        
        // Within same queue type, sort by creation time
    ...
    const nextTicket = allWaitingTickets[0];
    ...
    try {
      ...
      await ticketService.updateTicket(ticketId, {
        ...
        servedAt: now,
        waitTime,
        queueType: null, // Clear queue type when being served
        assignedToEmployee: null, // Clear assignment when being served
      });
  ...
  const handleCompleteTicket = async (ticketId: string, callNext: boolean = false) => {
    ...
    try {
      ...
      if (callNext) {
        // Use the auto-assign function to get the next ticket
        try {
  ...
  // Get waiting tickets with proper priority (personal queue first)
                ...
                {tabs.map((tab) => (
                  ...
                  >
                    <span className="text-xl">
                      {tab.icon === 'Clock' ? '🕐' : tab.icon === 'BarChart3' ? '📊' : '👤'}
                    </span>
